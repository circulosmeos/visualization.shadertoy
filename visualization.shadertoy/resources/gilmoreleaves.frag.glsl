...
float random (vec2 st) { 
    return fract(sin(dot(st.xy, vec2(1.9898,2.233)))* 3.5357);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float rndTime = randomRange(vec2(floor(iTime/flipTime), 7.345),3.,16.);
    ...
    //rand per column
    float rndColumn = random(vec2(colId, 6.890));
    ...
    float tileRnd = random(floor(uvs.xy * tileDims ) * 8.89 );
