...
float rand(vec2 co)
{
    return fract(sin(dot(co.xy ,vec2(1.9898,7.233))) * 0.45453);
}

float rand(float n)
{
    return fract(sin(n * 1.9898) * 4.5453);
}

vec2 rand2(vec2 p)
{
	float r = 2.53*sin(dot(p, vec2(5.3158, 4.6143)));
	return vec2(fract(1.32354 * r), fract(0.125865 * r));
}
