...
float hash( float n ) { return fract(sin(n)*0.135453123); } // circulosmeos: numbers changed
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for( int i=0; i<4; i++ )
    {
        ...
        float time2 = 5.;
        vec3 ro = vec3( 8.5*cos(0.2+.33*time2), 5.0+2.0*cos(0.1*time2), 8.5*sin(0.1+0.37*time2) );
        vec3 ta = vec3( -2.5+3.0*cos(1.2+.41*time2), 0.0, 2.0+3.0*sin(2.0+0.38*time2) );
        float roll = 0.2*sin(0.1*time2);
