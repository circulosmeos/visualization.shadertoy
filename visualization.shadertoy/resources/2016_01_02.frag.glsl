// https://www.shadertoy.com/view/MscGzs

#define iTime iGlobalTime/3.5
...
vec2 calcRayIntersection(vec3 rayOrigin, vec3 rayDir, float maxd, float precis) {
  ...
  for (int i = 0; i < 40; i++) { // circulosmeos: numbers changed
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  float bass = texture2D( iChannel0, vec2(400.,0.25) ).x;
  dist += (bass>.1)?bass*5.:0.0;
  rotation -= (bass>.1)?bass:0.0;
