// https://www.shadertoy.com/view/XsKfDW

#define iTime iGlobalTime*.20
...
float simplex(vec3 v){ 
  ...
  float amp = texture2D( iChannel0, vec2(400.,0.) ).x * 4.;
  //return 42.0 * dot(m*m, vec4(dot(p0,x0), dot(p1,x1), dot(p2,x2), dot(p3,x3)));
  return 42.0 * dot(m*m, vec4(dot(p0,x0), dot(p1,x1), dot(p2,x2), dot(p3,x3))) + amp;
}

// circulosmeos: code added
vec2 round_GLES( vec2 m ) {
    vec2 n;
    for(int i = 0; i < 2; i++)
        n[i] = floor(m[i] + .5);
        /*if ( fract(m[i]) > .5 )
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    float col = 0.0;
    for(int i = 0; i < 9; i++) // circulosmeos: numbers changed
    {
        // coordinates
        ...
        float r = pow(max(0.0, 0.5+0.5*simplex(vec3( round_GLES(0.5+uv/repeat)*(1.0/scale), 0.05*float(i)+T*0.77))),3.0);
